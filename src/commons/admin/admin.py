...
class HierarchyModelAdmin(admin.ModelAdmin):
    ...
    def _include_parent_url_arg(self, request, url):
        return querystring(url, includes={self.parent_url_arg: request.GET.get(self.parent_url_arg)})
    ...
    def get_parent_object(self, request):
        ...
        if not hasattr(self, _cached_prop):
            ...
            setattr(self, _cached_prop, get_object_or_404(self.parent_model, pk=parent_url_arg))
    ...
    def response_add(self, request, obj, post_url_continue=None):
        ...
        response['Location'] = self._include_parent_url_arg(request, response['Location'])
        return response
