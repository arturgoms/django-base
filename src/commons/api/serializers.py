    ...
    def validate(self, attrs):
        ...
        if validated_data['start_date'] > validated_data['end_date']:
            raise serializers.ValidationError({
                'start_date': _('The \'start_date\' must be less than or equal to \'end_date\'.')
            })
...
class PartialSerializerMixin:
    ...
    def _readable_fields(self):
        ...
        if not hasattr(self, _cached_prop):
            ...
            partial_fields = list(filter(lambda x: x.field_name in fields, readable_fields))

            for field in partial_fields:
                field_context = context.copy()
                field_context.update({
                    'fields': fields.get(field.field_name)
                })
...
def modelserializer_class_factory(model, fields, read_only=False, serializer=None, field_classes=None, meta=None):
    """
    ...
    serializer_class_attrs = {
        'Meta': meta_class,
        **{field_name: field_class() for field_name, field_class in (field_classes or {}).items()}
    }

    return type(serializer)(class_name, (serializer,), serializer_class_attrs)


def modelserializer_factory(model, fields, read_only=False, serializer=None, field_classes=None, meta=None, **kwargs):
    """
    Return a ModelSerializer containing form fields for the given model. You can
    optionally pass a `serializer` argument to use as a starting point for
    constructing the ModelSerializer.
    ...
    serializer_class = modelserializer_class_factory(
        model=model, fields=fields, read_only=read_only,
        serializer=serializer, field_classes=field_classes,
        meta=meta)
