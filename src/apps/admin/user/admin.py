...
class UserAdmin(PermissionsAdminMixin, SmartAdminMixin, admin.ModelAdmin):
    ...
    fieldsets = (
        ...
        (_('Status'), {
            'fields': ['date_joined', 'last_login', 'active_field']
        })
    )
    ...
    object_tools = {
        'change': [{
            ...
            'permission': lambda request, obj:
            obj.pk != request.user.pk and
            obj.status == models.User.Status.ACTIVE,
            'attrs': {'class': 'deletelink'}
        }, {
            'title': _('Unblock'),
            'url': lambda opts, obj: admin_url('unblock', opts=opts, args=[obj.pk]),
            'permission': lambda request, obj: obj.status == models.User.Status.BLOCKED
        }, {
            'title': _('Promote'),
            'url': lambda opts, obj: admin_url('promote', opts=opts, args=[obj.pk]),
            'permission': lambda request, obj:
                request.user.is_superuser is True and
                obj.pk != request.user.pk and
                obj.is_superuser is False
        }, {
            'title': _('Demote'),
            ...
            'permission': lambda request, obj:
                request.user.is_superuser is True and
                obj.pk != request.user.pk and
                obj.is_superuser is True
        }, {
            'title': _('Change Roles'),
            'url': lambda opts, obj: admin_url('change-role', opts=opts, args=[obj.pk]),
            'permission': lambda request, obj:
                request.user.is_superuser is True and
                obj.pk != request.user.pk
        }, {
            'title': _('Reset Password'),
            'url': lambda opts, obj: admin_url('reset-password', opts=opts, args=[obj.pk]),
            'permission': lambda request, obj:
                request.user.is_superuser is True and
                obj.pk != request.user.pk
        }],
    ...
    def invite_view(self, request):
        ...
        template_name = f'admin/{self.opts.app_label}/{self.opts.model_name}/invite_form.html'
        ...
        if request.method == 'POST' and form.is_valid():
            ...
            change_message = self.construct_change_message(request, form, None, add=True)
            self.log_addition(request, instance, change_message)
            return self.response_add(request, instance, next_url)

        return TemplateResponse(
            request, template=template_name, context={
                **self.admin_site.each_context(request),
                'title': _('Invite %(verbose_name)s') % {'verbose_name': self.opts.verbose_name},
                'media': self.media,
                'form': form,
    ...
    def change_role_view(self, request, object_id):
        ...
        """
        template_name = f'admin/{self.opts.app_label}/{self.opts.model_name}/change_role_form.html'
        instance = self.get_object(request, object_id)

        form = get_form(request, ChangeRolesAdminForm, initial={"roles": instance.roles}, context={'request': request})

        if request.method == 'POST' and form.is_valid():
            # change role for user.
            models.User.objects.set_roles(instance, form.cleaned_data['roles'])

            change_message = self.construct_change_message(request, form, None, add=False)
        ...
        return TemplateResponse(
            request, template=template_name, context={
                **self.admin_site.each_context(request),
                'title': _('Change Role for %(verbose_name)s') % {'verbose_name': self.opts.verbose_name},
                'media': self.media,
                'form': form,
                'fieldsets': Fieldsets(form, fieldsets=(
                    (_("Permissions"), {
    ...
    def block_view(self, request, object_id):
        ...
        """
        next_url = request.GET.get('next') or admin_url('change', self.opts, args=[object_id])
        template_name = f'admin/{self.opts.app_label}/{self.opts.model_name}/block_form.html'
    ...
    def promote_view(self, request, object_id):
        ...
        """
        template_name = f'admin/{self.opts.app_label}/{self.opts.model_name}/promote_form.html'
        instance = self.get_object(request, object_id)
        ...
        if request.method == 'POST':
            ...
            form = modelform_factory(models.User, fields=['is_superuser'])({'is_superuser': instance.is_superuser})
            change_message = self.construct_change_message(request, form, None, add=False)
            self.log_change(request, instance, change_message)
    ...
    def demote_view(self, request, object_id):
        ...
        """
        template_name = f'admin/{self.opts.app_label}/{self.opts.model_name}/demote_form.html'
        instance = self.get_object(request, object_id)
    ...
    def reset_password_view(self, request, object_id):
        ...
        if request.method == 'POST':
            ...
            form = modelform_factory(models.User, fields=['password'])({'password': instance.password})
            change_message = self.construct_change_message(request, form, None, add=False)
            self.log_change(request, instance, change_message)
