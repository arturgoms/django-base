...
class SettingsSchema(json_schema.JsonSchema):
    ...
    value = json_schema.MixedTypeProperty(types=[
        json_schema.StringProperty,
        json_schema.NumberProperty
    ], nullable=True)
...
class SettingsDetailApiTestCase(AuthenticatedUserAPITestCase):
    ...
    def test_should_retrieve(self):
        response = self.client.get(reverse('api:settings-detail', args=['example-settings']))
    ...
    def test_should_not_retrieve_if_not_authenticated(self):
        ...
        response = self.client.get(reverse('api:settings-detail', args=['session-price']))
